...
export function useMarkdownEditor({ editorRef, onContentChange }: UseMarkdownEditorProps) {
  const [blocks, setBlocks] = useState<MarkdownBlock[]>([]);
  const [isUpdating, setIsUpdating] = useState(false);
  const lastCaretPosition = useRef<{ blockId: string; offset: number } | null>(null);
  ...
  const saveCaretPosition = useCallback(() => {
    ...
    if (currentBlock) {
      ...
      if (selection && selection.rangeCount > 0) {
        ...
        const blockElement = editorRef.current.querySelector(`[data-block-id="${currentBlock.blockId}"]`);
        if (blockElement) {
    ...
    if (block.type === 'quote') {
      ...
      if (selection && selection.rangeCount > 0) {
        ...
        if (blockElement) {
          ...
          newBlocks.splice(currentBlock.blockIndex + (beforeCursor.trim() ? 1 : 0), 0, newParagraphBlock);
          
          setBlocks(newBlocks);
          
          // Set focus to the new paragraph after update
          setTimeout(() => {
            const newBlockElement = editorRef.current!.querySelector(`[data-block-id="${newParagraphBlock.id}"]`);
            if (newBlockElement) {
              const range = document.createRange();
              const selection = window.getSelection();
  ...
  // Apply slash command
  const applySlashCommandToCurrentBlock = useCallback((command: string) => {
    const currentBlock = getCurrentBlockFromCursor(editorRef.current!);
    if (!currentBlock) return;
