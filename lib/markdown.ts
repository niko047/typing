...
export interface MarkdownBlock {
  type: 'paragraph' | 'heading1' | 'heading2' | 'heading3' | 'quote' | 'divider';
  content: string;
  id: string;
}
...
export function renderBlocksToHTML(blocks: MarkdownBlock[]): string {
  return blocks
    .map((block) => {
      switch (block.type) {
        case 'heading1':
          return `<h1 class="text-3xl font-bold mb-4" style="font-family: var(--font-jost), sans-serif;" data-block-id="${block.id}">${escapeHtml(block.content)}</h1>`;
        case 'heading2':
          return `<h2 class="text-2xl font-bold mb-4" style="font-family: var(--font-jost), sans-serif;" data-block-id="${block.id}">${escapeHtml(block.content)}</h2>`;
        case 'heading3':
          return `<h3 class="text-xl font-bold mb-4" style="font-family: var(--font-jost), sans-serif;" data-block-id="${block.id}">${escapeHtml(block.content)}</h3>`;
        case 'quote':
          return `<blockquote class="border-l-4 border-muted-foreground/20 pl-4 italic my-4" data-block-id="${block.id}">${escapeHtml(block.content).replace(/\n/g, '<br>')}</blockquote>`;
        case 'divider':
          return `<hr class="my-8 border-muted-foreground/20" data-block-id="${block.id}">`;
        case 'paragraph':
        default:
          return block.content.trim() 
            ? `<p class="mb-4" data-block-id="${block.id}">${escapeHtml(block.content).replace(/\n/g, '<br>')}</p>`
...
export function getCurrentBlockFromCursor(editorElement: HTMLDivElement): { blockId: string; blockIndex: number } | null {
  ...
  while (element && element !== editorElement) {
    if (element.nodeType === Node.ELEMENT_NODE) {
      ...
      if (blockId) {
        ...
        const blockIndex = Array.from(allBlocks).findIndex(el => el.getAttribute('data-block-id') === blockId);
...
export function applySlashCommand(currentText: string, command: string): string {
